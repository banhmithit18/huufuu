...
function getNumberOfPage()
{
    ...
    if ($GLOBALS['category_id'] != 0) {
        $sql = "SELECT count(*) AS number_of_project FROM project JOIN image ON image.image_id = project.image_id JOIN category ON project.category_id = category.category_id WHERE project_status = 1 AND project.category_id = " . $GLOBALS['category_id'];
    } else {
